...
\begin{block}{Topic for Today ---副詞的用法---}
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item \kenten{目的}をあらわすto不定詞は、文の最後にも先頭にもきます
...
\visible<5->{{\small だが、特別に}


\myAnch{v}{white}{動詞}}%
\hspace{30pt}%
\begin{tabular}{ll}
 \visible<5->{\myAnch{n}{white}{名詞のはたらき}: }&\visible<6->{\fbox{To get up early} is important.}\\
 \visible<5->{\myAnch{adj}{white}{形容詞のはたらき}: }&\visible<7->{I want something \fbox{to drink}.}\\
 \visible<5->{\myAnch{adv}{white}{副詞のはたらき}: }&\visible<8->{She studied hard \fbox{to pass the exam}}.
\end{tabular}

\visible<5->%
{\begin{tikzpicture}[remember picture, overlay]
 \visible<5->{\draw[line width=2pt,opacity=.75, Maroon, ->] (v.east) to[out=0, in=180] (n.west);}
 \visible<5->{\draw[line width=2pt,opacity=.75, Maroon, ->] (v.east) to[out=0, in=180] (adj.west);} 
 \visible<5->{\draw[line width=2pt,opacity=.75, Maroon, ->] (v.east) to[out=0, in=180] (adv.west);} 
\end{tikzpicture}}
...
\mbox{}\hfill%
\visible<9->{%
\begin{minipage}{.26\textwidth}\scriptsize 
to不定詞：\Circled[fill color = white]{\,$\text{to} + \text{動詞の原形}$\,}
\end{minipage}
}

\hspace{50pt}{\small toは\kenten{述語動詞ではない}という目印！}
...
\duck[
signpost=\textcolor{black}{to},
%signpost=\scalebox{0.5}{
%\parbox{.5cm}{\color{black}
%to}},
