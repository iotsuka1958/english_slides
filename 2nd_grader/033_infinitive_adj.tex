...
\hspace{30pt}%
\begin{tabular}{ll}
 \visible<3->{\myAnch{n}{white}{名詞のはたらき}: }&\visible<3->{\fbox{To get up early} is important.}\\
 \visible<3->{\myAnch{adj}{white}{形容詞のはたらき}: }&\visible<3->{I want something \fbox{to drink}.}\\
 \visible<4->{\myAnch{adv}{white}{副詞のはたらき}: }&\visible<5->{She studied hard \fbox{to pass the exam}}.
\end{tabular}
...
{\begin{tikzpicture}[remember picture, overlay]
 ...
 \visible<4->{\draw[line width=2pt,opacity=.75, Maroon, ->] (v.east) to[out=0, in=180] (adv.west);} 
\end{tikzpicture}}

\vspace{-5pt}

\mbox{}\hfill%
\visible<6->{\begin{minipage}{.4\textwidth}
\visible<6->{to不定詞}：\Circled[fill color = yellow!50]{\,$\text{to} + \text{動詞の原形}$\,}
\end{minipage}}
