\documentclass[aspectratio=169,xcolor={dvipsnames,table}]{beamer}
\usepackage[no-math,deluxe,haranoaji]{luatexja-preset}
\renewcommand{\kanjifamilydefault}{\gtdefault}
\renewcommand{\emph}[1]{{\upshape\bfseries #1}}
\usetheme{metropolis}
\metroset{block=fill}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{blocks}[rounded][shadow=false]
\usecolortheme[rgb={0.7,0.2,0.2}]{structure}
%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Change alert block colors
%%% 1- Block title (background and text)
\setbeamercolor{block title alerted}{fg=mDarkTeal, bg=mLightBrown!45!yellow!45}
\setbeamercolor{block title example}{fg=magenta!10!black, bg=mLightGreen!70}
%%% 2- Block body (background)
\setbeamercolor{block body alerted}{bg=mLightBrown!25}
\setbeamercolor{block body example}{bg=mLightGreen!15}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{myPreamble4Slide.tex}
\usetikzlibrary{tikzmark}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{English is fun.}
\subtitle{She was listening to the radio.}
\author{}
\institute[]{}
\date[]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TEXT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}


\begin{frame}[plain]
  \titlepage
\end{frame}


\section*{授業の流れ}
\begin{frame}[plain]
  \frametitle{授業の流れ}
  \tableofcontents
\end{frame}

\section{過去進行形}

\subsection{過去進行形とは}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{過去進行形}
 \begin{enumerate}
 \item \visible<1->{She \textcolor{ForestGreen}{\bfseries is sleeping} now. {\small 彼女はいま眠っています。\hfill{}(現在進行形)}}
\visible<2->{\signchart[width=10]{,,{\textcolor{ForestGreen}{いま}}}{,,,}}
 \item \visible<3->{She \textcolor{Maroon}{\bfseries was playing} tennis two hours ago.{\small 彼女は2時間前テニスをしていた。\\\mbox{}\hfill{}(過去進行形)}}
\visible<4->{\signchart[width=10,height=.5]{{\textcolor{Maroon}{{2時間前}}},,いま}{,,,}}
\end{enumerate}

\begin{tikzpicture}[overlay]
 %\draw[gray!50] (0,0) grid (12,5);
 \visible<2->{\draw[ForestGreen!70,line width=6pt,opacity=.5] (9.2,2.95) --(10.6,2.95);}
\visible<4->{ \draw[Maroon!70,line width=6pt,opacity=.7] (4.2,.9) -- (5.6,.9);}
\end{tikzpicture}

\visible<5->{{\small 
\mbox{}\hfill{%
}$\left\{\begin{tabular}[c]{l}
         was\\
         were
       \end{tabular}\right\} + \text{--ing}$
}
}


\vspace{10pt}

\begin{block}<6->{Topics for Today}
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item 現在進行形(〜している)$\longrightarrow$今この瞬間に起こっていること
 \item 過去進行形(〜していた)$\longrightarrow$過去のある時点で起こっていること%
\hfill{\tiny 0115}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_01.mp3}}
\end{itemize}
     \end{block}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises 1}

{\small 1は正しい方を選びましょう。2〜4は日本語の意味になるよう(~~~~~)内の語句を並べ替えましょう。先頭に来る語は大文字で書き始めてください}
 \begin{enumerate}
  \item Bob ( studies / is studying / \alt<2->{\Circled[outer color = BurntOrange]{ was studying }}{was studying} ) English last night.
  \item ( is / she / sleeping ) now.{\small 彼女はいま眠っています。}\\
\visible<3->{She is sleeping now.}
  \item ( I / waiting / was ) for the train then.{\small 私はそのとき電車を待っていました。}\\
	\visible<4->{I was waiting for the train then.}%
\hfill{\scriptsize wait for ～を待つ}
  \item ( eating / dinner / was / with  / I  / my family ) at that time.\\
\mbox{}\hfill{}{\small 私はそのとき家族と夕飯を食べていました}。\\
\visible<5->{I was eating dinner with my family at that time.}%
 \end{enumerate}
\hfill{\tiny 0209}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_02.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{--ingのつくりかた（復習）}
 
\begin{center}
% \rowcolors{2}{NavyBlue!50}{yellow!40}
\begin{tblr}{
  colspec = {l l l l}, % 列の仕様
  row{5-6} = {bg=yellow!50}, % 1行目の背景色を黄色に設定
  row{7-8} = {bg=NavyBlue!40} % 3行目の背景色を薄い灰色に設定
}
\toprule
&{\small 原形}&{\small --ing}\\\midrule
1&\visible<1->{eat}&\visible<2->{{\small eating}}&\visible<2->{\scriptsize ingをつけるだけ (原則)}\\
2&\visible<1->{play}&\visible<3->{{\small playing}}&\\
3&\visible<1->{go}&\visible<4->{{\small going}}&\\
4&\visible<1->{make}&\visible<5->{{\small making}}&\visible<5->{\scriptsize eをとって--ing}\\
5&\visible<1->{write}&\visible<6->{{\small writing}}&\visible<6->{\scriptsize use $\rightarrow$\,using}\\
6&\visible<1->{swim}&\visible<7->{{\small swimming}}&\visible<7->{\scriptsize 子音字を重ねて--ing}\\
7&\visible<1->{run}&\visible<8->{{\small running}}&\visible<8->{\scriptsize sit $\rightarrow$\,sitting \ldots{}}\\
\bottomrule
\end{tblr}%
\end{center}
\hfill{\tiny 0246}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_04.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises 2}

{\small 適当な動詞を(~~~~~)内に補い、英文を完成させましょう。必要に応じて適当な形に変えてください}

\begin{columns}[t]
\begin{column}{.75\textwidth}
 \begin{enumerate}
  \item He was (~\alt<2->{playing}{\phantom{playing}}~) soccer at that time.
  \item She was  (~\alt<3->{making}{\phantom{making}}~) a delicious dinner for us then.
  \item I was  (~\alt<4->{writing}{\phantom{writing}}~) a letter to my friend at that time.
  \item They were  (~\alt<5->{swimming$^{1}$}{\phantom{swimming }}~) in the pool then.
  \item The dog was  (~\alt<7->{running$^{2}$}{\phantom{running }}~) in the yard at that time. \\
\hfill{}\visible<6->{{\scriptsize 1\visible<8->{,2} playingでもOK}}\hfill\mbox{}
 \end{enumerate} 
\end{column} 
\begin{column}{.225\textwidth}
{\small 次の中から\\選んでください}
 \begin{tcolorbox}
  swim\\
  run\\
  make\\
  write\\
  play
 \end{tcolorbox}
\end{column}
\end{columns}

\visible<1->{\scriptsize play（遊ぶ）　play the guitar（ギターを弾く）　play tennis（テニスをする）}

\hfill{\tiny 0332}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_03.mp3}}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{過去進行形の否定}
%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{否定を表すnot}
 \Large

否定を表すことば： {\LARGE\bfseries not}\hspace{20pt}\textipa{/n\'At/}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{過去進行形の否定}
 \begin{enumerate}
  \item \begin{enumerate}
	 \item<1-> Bob was hungry.
	 \item<2-> Bob was \myEmph[2-]{Maroon}{not} hungry.
	\end{enumerate}
  \item \begin{enumerate}
	 \item<3-> Jane \myEmph[3-]{NavyBlue}{was waiting} for the train then.
	 \item<4-> Jane \myEmph[3-]{NavyBlue}{was} \myEmph[4-]{Maroon}{not} \myEmph[3-]{NavyBlue}{waiting} for the train then.
	\end{enumerate}
 \end{enumerate}

\begin{block}<5->{Topics for Today}\small
\begin{itemize}\setbeamertemplate{items}[square]
 \item  過去進行形\,\,\Circled[fill color = white]{\,\,$\text{be動詞}+\text{---ing}$\,\,}\,\,の否定$\longrightarrow$\,\,%
\Circled[fill color = white]{\,\,$\text{be動詞} + \text{\textcolor{Maroon}{\bfseries not}} + \text{---ing}$\,\,}
 \visible<6->{\item 短縮形wasn't ---ing, weren't ---ingが使われることもあります} \end{itemize}
      \end{block}
\hfill{}\onslide<7->{\scriptsize be動詞の直後に\textcolor{Maroon}{\bfseries not}がくるのは、be動詞の否定と同じです\hspace{1\zw}}


\hfill{\tiny 0153}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_05.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{過去進行形の疑問文}
\begin{frame}[plain]{過去進行形の疑問文}
  \begin{enumerate}
  \item \begin{enumerate}
	 \item<1-> Bob was hungry.
	 \item<2-> Was Bob hungry?
	\end{enumerate}
  \item \begin{enumerate}
	 \item<3-> Jane \myEmph[3-]{NavyBlue}{was waiting} for the train then.
	 \item<4-> \myEmph[3-]{NavyBlue}{Was} Jane \myEmph[3-]{NavyBlue}{waiting} for the train then?
	 \item<5-> Yes, she was.\\
                   No, she was not.\\
                   No, she wasn't.
	\end{enumerate}
 \end{enumerate}

\begin{block}<6->{Topics for Today}
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item 疑問文のつくり方: be動詞を先頭に
\hfill{}{{\bfseries Was} she {\bfseries sleeping} then?}
 \item 疑問文への答え方
\hfill{}Yes, she was.\\
\hfill{}No, she was not.\\
\hfill{}($=$ No, she wasn't.)
\end{itemize}
\end{block}

\hfill{\tiny 0251}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_06.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises}

{\small ［~~~~~］内の指示にしたがって英文を書き換えましょう。
3～5は疑問文に書き換え、さらにそれに対する答えを「はい」と「いいえ」の2とおりつくりましょう}\hfill{\tiny 0524}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_07.mp3}}

\begin{enumerate}
 \item Bob was playing the violin this morning. {\scriptsize ［否定文にする］}\\
       \visible<2->{Bob was not playing the violin this morning.}
 \item We were studying science at that time. {\scriptsize ［否定文にする］}\\
       \visible<3->{We were not studying science at that time.}
 \item He was walking in the park then. {\scriptsize ［疑問文にする］}\\
       \visible<4->{Was he walking in the park then?}
       \visible<5->{--- Yes, he was. / No, he was not ($= \text{wasn't}).$}
 \item She was swimming in the pool. {\scriptsize ［疑問文にする］}\\
       \visible<6->{Was she swimming in the pool?}
       \visible<7->{--- Yes, she was. / No, she was not ($= \text{wasn't}).$}
 \item The cats were sleeping then. {\scriptsize ［疑問文にする］}\\
       \visible<8->{Were the cats sleeping then?}
       \visible<9->{--- Yes, they were. / No, they were not\,\makebox[0pt][l]{($= \text{weren't}$).}}

\end{enumerate}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Listen, Then Read\,\,\,\,\,{\tiny 0029}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_reading.mp3}}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain,t]{Exercises --- True or False? ---}
\vspace{-8pt}
\begin{tcolorbox}[colframe=ForestGreen,
  colback=NavyBlue!10!white,
  colbacktitle=NavyBlue!40!white,
  coltitle=black, %fonttitle=\bfseries,
before upper={\setlength{\parindent}{1.25em}},
 title={A Busy Sunday Afternoon\hfill{\tiny 0029}\quad{\scriptsize \myaudio{./audio/005_was_ing_intro_reading.mp3}}}
]
\noindent{}Last Sunday afternoon, Ken and his family were staying at home because of rain.

At two o'clock, Ken was reading a comic book in his room.
His sister, Emma, was listening to music in the living room.

What were their parents doing?
Their father was cleaning the kitchen, and their mother was making a cake.

At three o'clock, the cake was ready.
They were all eating delicious cake and talking happily together.
\end{tcolorbox}

\vspace{-5pt}
%\visible<2->{\small 次の各文が本文の内容とあっていればT,そうでなければFと答えましょう}
%\vspace{-11pt}
\begin{enumerate}\setlength{\itemsep}{-3.4pt}
\item<2-> Ken and his family were staying at home last Sunday afternoon.\hfill\visible<3->{T}
\item<2-> Ken was listening to music at two o'clock.\hfill\visible<4->{F}
\item<2-> Ken's father was cleaning the kitchen.\hfill\visible<5->{T}
\item<2-> Ken's mother was making cookies.\hfill\visible<6->{F}
\item<2-> The family were eating cake together at three o'clock.\hfill\visible<7->{T}
\end{enumerate}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{過去進行形のまとめ}
%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{要点}
 
\begin{block}{was / were $+$ --ing}
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item 過去進行形(〜していた)$\longrightarrow$過去のある時点で起こっていること\\%
\hfill{}She {\bfseries was sleeping} then.
 \item 否定文のつくり方: be動詞の直後にnot\hfill{}She {\bfseries was} \Circled[fill color = white]{\,not\,} {\bfseries sleeping} then.
 \item 疑問文のつくり方: be動詞を先頭に\hfill{}{\bfseries Was} she {\bfseries sleeping} then?
 \item 疑問文への答え方
\hfill{}Yes, she was.\\
\hfill{}No, she was not.\\
\hfill{}($=$ No, she wasn't.)
\end{itemize}
\hfill{{\scriptsize 否定文のつくり方・疑問文のつくり方・疑問文への答え方--みんなbe動詞の時と同じ}}
\end{block}

\hfill{\tiny 0229}\,{\scriptsize \myaudio{./audio/005_was_ing_intro_08.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{ひとつ確認}
%%%%%%%%%%%%%%%%%%%%%
\begin{frame}<-6>[plain]{最後の1文字を重ねるのか重ねないのか}

{\small カッコ内の動詞を適当な形にしてください}

\begin{enumerate}
 \item \begin{enumerate}
	\item He was \alt<1>{(~~~~eat~~~~)}{(~~eating~~)} lunch at that time.\hfill{}eat \textipa{/\'\i:t/}
	\item He was \alt<1-2>{(~~~~~sit~~~~~)}{(~~sitting~~)} on a chair at that time.\hfill{}\tikzmark{A}sit \textipa{/s\'It/}
       \end{enumerate}
 \item \begin{enumerate}
        \item She was \alt<1-3>{(~~~~learn~~~~)}{(~~learning~~)} Japanese.%
\hfill{}learn \textipa{/l\'\textrhookschwa :n/}
         \item She was \alt<1-4>{(~~~~~run~~~~~)}{(~~running~~)} in the yard.%
\hfill{}\tikzmark{B}run \textipa{/r\'\textturnv n/}
       \end{enumerate}
\end{enumerate}

\hfill{\textipa{:}\,\,\,長音記号}

\begin{block}<6->{Topics for Today}
\Circled[fill color=white]{\,\,$\text{短母音}+\text{子音字}$\,\,}\,\,で終わる動詞のとき\tikzmark{Z}、最後の１文字を重ねる\,\,\dbend\\
\mbox{}
\end{block}


\begin{tikzpicture}[remember picture,overlay]
 \visible<6->{\draw[<-,opacity=.4,line width=2pt] ([xshift=-2pt,yshift=2pt]pic cs:A) to[bend right] ([xshift=2pt, yshift=4pt] pic cs:Z);}
 \visible<6->{\draw[<-,opacity=.4,line width=2pt] ([xshift=-2pt,yshift=2pt]pic cs:B) to[bend right] ([xshift=2pt, yshift=4pt] pic cs:Z);}
\end{tikzpicture}

\end{frame}
%%%%%%%%%%%%
\begin{frame}[plain]{Exercises}\large

{\small つぎの動詞を--ing形にしてみましょう}

\begin{enumerate}
 \item cut \textipa{/k\'\textturnv t/} \visible<2->{$\longrightarrow$\,\,cutting}
 \item get \textipa{/g\'et/} \visible<3->{$\longrightarrow$\,\,getting}
 \item forget \textipa{/f\textrhookschwa g\'et/} \visible<4->{$\longrightarrow$\,\,forgetting}
 \item hit \textipa{/h\'It/} \visible<5->{$\longrightarrow$\,\,hitting}
 \item put \textipa{/p\'Ut/} \visible<6->{$\longrightarrow$\,\,putting}
 \item meet \textipa{/m\'\i:t/} \visible<7->{$\longrightarrow$\,\,meeting}
 \item begin \textipa{/bIg\'In/} \visible<8->{$\longrightarrow$\,\,beginning}
 \item win \textipa{/w\'In/} \visible<9->{$\longrightarrow$\,\,winning}
 \item learn \textipa{/l\'\textrhookschwa :n/} \visible<10->{$\longrightarrow$\,\,learning}
\end{enumerate}

\vspace{-80pt}

\hfill%
{\begin{tikzpicture}
\duck[tshirt,
jacket=gray,
bowtie,
%crazyhair,
speech={\tiny 重ねる？},
laughing,
signpost=\scalebox{0.4}{
\parbox{2cm}{
長母音か\\
短母音か\\で決まる}},
]
\end{tikzpicture}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
 
\includegraphics[width=1.01\textwidth]{./infographic/005_was_ing_intro_infographic.png}
\hfill{\tiny audio\_overview 0833}\,{\scriptsize \myaudio{./audio/overview/005_was_ing_intro_audio_overview.m4a}}%
\hfill{\tiny eduop\_boys 0306}\,{\scriptsize \myaudio{./audio/manzai/005_was_ing_intro_manzai.mp3
}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%
\begin{frame}[plain]{Check for Understanding}
 
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/005_was_ing_intro_test.png}
 \end{column}
 \begin{column}{0.59\textwidth}
  \begin{itemize}
   \item[\textbullet{}] エラーが出る場合は\\
eduopちばのアカウント\\\hfill{}(xxxxxx@eduop-chiba.jp)\\
で、はいりなおしてください
  \end{itemize}
 \end{column}
\end{columns}

\vspace{-8pt}

\url{https://forms.gle/mv96M4CDfyynkE3L8}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%以下、追加すること
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{疑問詞をもちいて}
 \begin{enumerate}
  \item What was she doing at that time? --- She was doing homework.
  \item Where was he running? --- In the park.
	\mbox{}\hfill{}cf. He was running in the park.
  \item Who was watching television? --- John was.
	\mbox{}\hfill{}cf. John was watching television.
 \end{enumerate}

\visible<5->{%
\begin{exampleblock}{Topics for Today}
\pause
\begin{itemize}\small
 \item $\text{疑問詞} + \text{be動詞} + \text{S} + \text{--ing\ldots ?}$
 \item ただし、疑問詞が主語のとき\\
\mbox{}\hspace{40pt}$\text{疑問詞($=$S)} + \text{be動詞} + \text{--ing\ldots ?}$

\end{itemize}
     \end{exampleblock}
}
\hfill\myaudio{./audio/005_was_ing_intro_08.mp3}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises}
(~~~~~)の語句を並べ替えて対話を完成させましょう。文頭にくる語は大文字で書きはじめてください
\begin{enumerate}
 \item ( doing / you / were/ what ) last night?
--- I was studying history. \\
\visible<2->{What were you doing last night?}
 \item ( was / what / doing / your brother ) then?
--- He was cleaning his room.\\
\visible<3->{What was your brother doing then?}
 \item ( was / the guitar / playing / who )? --- Paul was.\\
\visible<4->{Who was playing the guitar?}
\end{enumerate}
\hfill\myaudio{./audio/005_was_ing_intro_09.mp3}
\end{frame}
%%%%%%%%%%%
\begin{frame}[plain]{Check for Understanding}
 
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/005_was_ing_intro_test.png}
 \end{column}
 \begin{column}{0.59\textwidth}
  \begin{itemize}
   \item[\textbullet{}] エラーが出る場合は\\
eduopちばのアカウント\\\hfill{}(xxxxxx@eduop-chiba.jp)\\
で、はいりなおしてください
  \end{itemize}
 \end{column}
\end{columns}

\vspace{-8pt}

\url{https://forms.gle/mv96M4CDfyynkE3L8}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
