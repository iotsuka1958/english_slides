...
\visible<1->{I \alt<1-2>{\myAnch{aux1}{white}{must}}{\myAnch{AUX1}{Orange}{must}}  \textcolor{Green}{study} science.}
\hspace{80pt}\visible<2->{{\footnotesize science: 理科、科学}}


\vspace{15pt}

\visible<4->{I \alt<1-4>{\myAnch{aux2}{white}{have to}}{\myAnch{AUX2}{Orange}{have to}}  \textcolor{Green}{study} science.}
...
\begin{itemize}\small
 \item   「〜しなければならない」はmustのほかに\\
{\,\,\,}\fcolorbox{BurntOrange}{white}{have to $+$ 動詞($=\text{動詞の原形}$)}
 \end{itemize}
     \end{exampleblock}
...
\visible<2->{She has to write a report.}
  \item ベンは歯医者に行かなければならない。
Ben (the dentist / to / to /has / go )\\
\visible<2->{Ben has to go to the dentist.}  
\item ベティとジャックは家を掃除しなければならない。\\
Betty and Jack  ( clean /  have / the house / to )\\
\visible<2->{Betty and Jack have to clean the house.}
 \end{enumerate}
\end{frame}
...
\mbox{}\hspace{35pt}You have to study tonight. \pause
\hspace{80pt}{\footnotesize tonight: 今晩}\pause\hspace{-140pt}\raisebox{-5pt}{\scalebox{1.4}{\myDownwardPitch}}
\pause

\vspace{15pt}

\myAnch{do}{orange}{Do} you have to study tonight \myAnch{question}{orange}{?}
\pause
...
\begin{itemize}\small
 \item   You have to--の疑問文「〜しなければいけませんか」\\
\mbox{}\hfill{}$\text{Do you have to} + \text{動詞の原形}$\ldots{}\,\,\,?　　 \end{itemize}
     \end{exampleblock}
...
\mbox{}\hspace{50pt}\alt<2-3>{You}{He} \alt<1-4>{have to}{\myAnch{T1}{orange}{has to}} study hard. \only<4>{\ding{55}}
...
\onslide<8->{%
\myAnch{does}{orange}{Does} he \fcolorbox{orange}{white}{have to} study hard\myAnch{question}{orange}{?}
}
\onslide<9->{%
\scalebox{1.4}{\myRisingPitch}
}
...
\pause
\begin{itemize}\small
 \item   主語が三人称単数のとき$\text{Does} + \text{S} + \text{have to} + \text{動詞の原形}$\,\,\ldots{}\,\,\,?\pause
 \item   文末に`?'をつける（イントネーションは\myRisingPitch{}）
\end{itemize}
     \end{exampleblock}
\end{frame}
...
つぎの文を疑問文にしましょう。
 ...
 \begin{enumerate}
  ...
  \item<1-> She has to wake up early tomorrow.\hspace{12pt}%
        \onslide<7->{$\longrightarrow$\,\,\,\,\, Does she have to wake up early?}
...
\visible<1->{You \fcolorbox{BurntOrange}{white}{have to} finish it today.}

\vspace{8pt}

\visible<2->{You \textcolor{BrickRed}{{\bfseries don't}} \fcolorbox{BurntOrange}{white}{have to} finish it today.}


\visible<3->{\fbox{きょう終わらせなければならない}\,というわけではない}\\
\visible<4->{$\longrightarrow$\,\,「終わらせなくてもいい」}
