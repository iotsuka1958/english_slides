...
\begin{frame}[plain]{現在完了とは(復習)}
 \begin{enumerate}
 \item Jane \textcolor{NavyBlue}{\bfseries has stayed} in London for six years.（継続）
 \item I \textcolor{NavyBlue}{\bfseries have watched} the movie three times.（経験）
...
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{現在完了--結果・完了--}
 
\visible<1->{\begin{enumerate}%\setcounter{enumi}{1}
 \item $\left\{\begin{tabular}{rl}
(A)& I \textcolor{Maroon}{\bfseries lost} my bag.\hspace{12\zw}{\scriptsize lost: lose\,(失くす)の過去形} \textipa{/l\'O:st/}\\
...
My father ( \alt<4->{\Circled[outer color=orange]{lost}}{lost} / has lost / loses ) his bag \myEmph[3-]{Maroon}{yesterday}.
 \item {\small 彼はフランスに行ってしまった。}{\footnotesize (今もフランスにいる)}\\
He ( have gone / \alt<5->{\Circled[outer color=orange]{has gone}}{has gone} / went  ) to France.
 \item {\small 彼女は2年前にイタリアに行きました。\textdbend}\\
She ( \alt<6->{\Circled[outer color=orange]{went}}{went} / has gone ) to Italy \myEmph[5-]{Maroon}{two years ago}.
\end{enumerate}
...
\small
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item<6-> \textbf{just}（たった今、ちょうど）、\textbf{already}（すでに）、\textbf{yet}（否定文で「まだ（～ない）」、疑問文で「もう」）を現在完了形と組み合わせることがあります
 \item<7-> \textbf{just}や\textbf{already}は\textbf{have}のあとにきます%
\hspace{70pt}\textbf{have} $\left\{
	...
	\textbf{already}
       \end{tabular}
\right\}$ 過去分詞
 \item<8-> 否定文や疑問文の\textbf{yet}は文末にきます
\end{itemize}
      \end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises}
 あたえられた動詞と副詞をもちいて英語で表現してください。あたえられた動詞を現在完了形にしましょう

\begin{enumerate}
 \item 彼女は朝食をもう食べましたか( eat / yet )\hfill{\scriptsize breakfast \textipa{/br\'ekf@st/} 朝食}\\
\visible<2->{Has she eaten breakfast yet?}
 \item 彼はまだ昼食を食べていません( eat / yet )\hfill{\scriptsize lunch \textipa{/l\'\textturnv ntS/} 昼食}\\
\visible<3->{He has not eaten lunch yet.}
 \item わたしはたったいま宿題を終えたところだ( finish  / just )\\
\visible<4->{I have just finished my homework.}\hfill{\scriptsize homework \textipa{/h\'oUmw\`\textrhookschwa :k/} 宿題}
 \item その会議はもう始まっています( start / already )\hfill{\scriptsize meeting \textipa{/m\'\i:tIN/} 会議}\\
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[行ってしまった（結果・完了）\\行ったことがある（経験）]{行ってしまった（結果・完了）\\行ったことがある（経験）}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain,t]{行ってしまった / 行ったことがある}

\begin{enumerate}
 \item<1-> She \textbf{went to} Hawaii.\hfill{\scriptsize （過去の事実）}
 \item<2-> She \textbf{has gone to} Hawaii.\hfill{\scriptsize （結果・完了）}
 \item<3-> She \textbf{has been to} Hawaii.\hfill{\scriptsize （経験）}
 \item<4-> \textbf{Have} you \myEmph{Maroon}{ever} \textbf{been to} Hawaii?\hfill{\scriptsize （経験の有無をたずねる）}
 \item<5-> I \textbf{have} \myEmph{Maroon}{never} \textbf{been to} Hawaii.\hfill{\scriptsize （未経験）}
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
\section{Listen, Then Read\,\,\,\,\,{\tiny 0047}\,{\scriptsize \myaudio{./audio/014_have_pp_kekka_reading.mp3}}}
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
 
\includegraphics[width=1.01\textwidth]{./images/nanobanana-output/014_have_pp_kekka_reading.png}

\vspace{-15pt}

\hfill{\tiny 0047}\,{\scriptsize \myaudio{./audio/014_have_pp_kekka_reading.mp3}}
...
before upper={\setlength{\parindent}{1.25em}},
 title=Lunch Break\mbox{}\hfill{\tiny 0047}\,{\scriptsize \myaudio{./audio/014_have_pp_kekka_reading.mp3}}
]

\noindent{}Where is Bob?
He is not in the classroom now.
He has gone to the library.
He has lost his English dictionary\footnotemark[1], so\footnotemark[2] he is looking for\footnotemark[3] it there.

I have just finished my lunch.
I am full\footnotemark[4] now.
I have already done\footnotemark[5] my math homework.
It was easy.
However\footnotemark[6], I haven't started my English report yet.
It is due\footnotemark[7] tomorrow!
Have you finished your homework yet?

\vspace*{8pt}

\scriptsize
$^{1}$dictionary \textipa{/d\'IkS@n\`eri/}\,\,辞書\hfill%
$^{2}$so だから\hfill%
$^{3}$look for ～をさがす\hfill%
$^{4}$full：おなかがいっぱいで

$^{5}$do one's homework 宿題をする\hfill%
$^{6}$however しかし\hfill%
$^{7}$due \textipa{/dj\'u:/}\,\,期日の、提出日で
\end{tcolorbox}
...
\item<2-> Bob is not in the classroom right now.\hfill\visible<3->{T}
 \item<2-> The speaker is eating lunch now.\hfill\visible<4->{F}
 \item<2-> The speaker has already finished the math homework.\hfill\visible<5->{T}
 \item<2-> The speaker has to finish the English report.\hfill\visible<6->{T}
\end{enumerate}
...
\begin{tcolorbox}[title=昼休み,
before upper={\setlength{\parindent}{1\zw}}
]
ボブはどこですか？
彼は今、教室にいません。
彼は図書室へ行ってしまいました。
彼は英語の辞書をなくしてしまったので、そこでさがしています。

わたしはたったいま、昼食を食べおえたところです。
だから、お腹がいっぱいです。
わたしはすでに数学の宿題をすませてしまいました。
それはかんたんでした。
しかし、わたしはまだ英語のレポートをはじめていません。
明日が提出日です！
あなたはもう宿題をおわらせてしまいましたか？
\end{tcolorbox}
\end{frame}
...
\includegraphics[width=1.01\textwidth]{./infographic/014_have_pp_kekka_infographic.png}

{\tiny audio\_overview 2243}\,{\scriptsize \myaudio{./audio/overview/014_have_kekka_audio_overview.m4a}}%
\hfill{\tiny eduop\_boys 0321}\,{\scriptsize \myaudio{./audio/manzai/014_have_kekka_manzai.mp3}}
...
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/014_have_pp_kekka_test.png}
 \end{column}
...
\url{https://forms.gle/c6NjN7AJQjsopnej8}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
