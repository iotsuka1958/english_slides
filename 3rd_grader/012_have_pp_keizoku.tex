...
\visible<5->{for six yearsは「期間」を表す表現です}\mbox{}\hfill{\scriptsize \myaudio{./audio/012_have_pp_keizoku_02.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
...
\visible<4->{{\bfseries for} six daysは\kenten{期間}を表します}\mbox{}\hfill\myaudio{./audio/012_have_pp_keizoku_03.mp3}
...
\visible<5->{{\bfseries since} 〜は\kenten{起点}を表します。「〜から、以来」}\mbox{}\hfill{\scriptsize \myaudio{./audio/012_have_pp_keizoku_04.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We \alt<1>{(~~\phantom{have}~~)}{(~~have~~)} enjoyed the game for three hours. \item {\small ビリーはピアノの練習をはじめて2年になる。}\hfill{}{\scriptsize practice \textipa{/pr\'\ae ktIs/} 練習する}\\
...
I have \alt<1-4>{(~~\phantom{been}~~)}{(~~been~~)} sick \alt<1-5>{(~~\phantom{since}~~)}{(~~since~~)} this morning.\hfill\visible<6->{{\scriptsize since $+$ 名詞}}
 \item {\small 彼が子供のころから、わたしは彼を知っています。}\\
I have \alt<1-6>{(~~\phantom{known}~~)}{(~~known~~)} him \alt<1-7>{(~~\phantom{since}~~)}{(~~since~~)} he was a child.\hfill\visible<8->{{\scriptsize since \Circled{\,\,S $+$ V\,\,}}}
\end{enumerate}

\begin{exampleblock}<9->{Topics for Today}\small
\begin{itemize}\setbeamertemplate{items}[square]
 \item \visible<10->{{\bfseries for} 〜は\kenten{期間}を表します\hfill{}{\scriptsize {\bfseries for} two days, {\bfseries for} three months, {\bfseries for} a long time \ldots}}
 \item \visible<11->{{\bfseries since} 〜は\kenten{起点}を表します\hfill{}{\scriptsize {\bfseries since} yesterday, {\bfseries since} 2000, {\bfseries since}  he was a child \ldots}}
...
\mbox{}\hfill{\scriptsize \myaudio{./audio/012_have_pp_keizoku_07.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{疑問文と答え方}
...
\hfill\visible<4->{{\small どれくらいの期間}}
...
\begin{exampleblock}<5->{Topics for Today}\small
\begin{itemize}\setbeamertemplate{items}[square]
 \item $\text{How long have} + \text{S} + \text{過去分詞 \ldots ?}$は「継続している期間」をたずねる表現\\
