...
\visible<3->{\draw[NavyBlue, line width=2pt,->] (aux1.south) to[out=-90,in=90] (aux2.north);}
\end{tikzpicture}
...
\mbox{}\hspace{0\zw}%
\visible<7->{主語が三人称単数のときは$\text{Has} + \text{S} + \text{過去分詞\ldots{\,?}}$}
 \item \visible<8->{答え方はYes, S $+$ have.またはNo, S $+$ have not($=$haven't).}\\
\visible<9->{主語が三人称単数のときはYes, S $+$ has.またはNo, S $+$ has not($=$hasn't).}
\end{itemize}
      \end{exampleblock}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{How long have you 〜?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{How long have you 〜?}

\mbox{}\hspace{80pt}\visible<1->{You \myAnch{has1}{NavyBlue}{have} stayed in Paris for two weeks.}
 
\vspace{10pt}

\mbox{}\phantom{~~How long~~}\visible<2->{\myAnch{has2}{NavyBlue}{Have} you stayed in Paris \myAnch{long1}{Maroon}{for two weeks}?}

\vspace{10pt}

\visible<4->{\myAnch{long2}{Maroon}{How long} have you stayed in Paris?}%
\hfill\visible<6->{{\small どれくらいの期間}}

\begin{tikzpicture}[remember picture,overlay]
 \visible<3->{\draw[NavyBlue,->,line width=2pt] (has1.south) to[out=-90,in=90] (has2.north);}
 \visible<5->{\draw[Maroon,->,line width=2pt] (long1.south) to[out=-165,in=20] (long2.north);}
\end{tikzpicture}
...
\mbox{}\hfill\visible<9->{「どのくらいのあいだ(期間)〜ですか」　　　}
 \item \visible<10->{主語が三人称単数のときは$\text{How long has} + \text{S} + \text{過去分詞 \ldots ?}$}
