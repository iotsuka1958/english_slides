...
(B)& I \textcolor{NavyBlue}{\bfseries have visited} the tower three times.\hspace{7.6\zw}{\scriptsize three times 三回、三度}
\end{tabular}
\right.$
\end{enumerate}}
...
\visible<2->{ \fill[Maroon!90,opacity=.75] (8.675,3.72) circle [radius=.25];}
...
$\left\{\begin{tabular}{l}
 ...
 many times\\
\end{tabular}\right\}$
は、\kenten{経験}をあらわす現在完了とセットでよく使います
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{neverとever}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\large
 ...
 \begin{enumerate}
  \item<1-> I have \textcolor{Maroon}{\bfseries never} eaten sushi.\hfill\visible<2->{{\scriptsize eat---ate---eaten}}
  \item<4-> Have you \textcolor{NavyBlue}{\bfseries ever} climbed a mountain?\hfill{}\visible<5->{{\scriptsize climb---climbed---climbed 登る}}
 \end{enumerate}
...
\begin{itemize}\setbeamertemplate{items}[square]\small
 \item<3-> {\bfseries never}は「今までに一度もない」(\kenten{未経験})\\
I {\bfseries have never} $+$ 過去分詞 \ldots\,\,\,\textbf{.}{{\footnotesize （今までに一度も〜したことがない）}}
 \item<6-> {\bfseries ever}は疑問文で用いて「今までに」\hfill{\kenten{経験の有無}をたずねる}\\
\textbf{Have you ever} $+$ 過去分詞 \ldots\,\,\,?{{\footnotesize （今までに〜したことがありますか）}}
\end{itemize}
\end{block}
\end{frame}
...
\begin{tikzpicture}[remember picture,overlay]
 \visible<4->{\draw[Maroon,->,line width=3pt,opacity=0.5] (long1.south) to[out=-90,in=90] node[sloped,above,text=black,font=\tiny,pos=.5]{How many timesに置き換え先頭へ} node[sloped,below,text=black,font=\tiny,pos=.5]{後ろは疑問文の語順} (long2.north);}
\end{tikzpicture}
...
\begin{enumerate}
    % Q1: read (過去分詞)
    \item あなたはこの本を何回読みましたか。\\
    How many times have you \underline{\hspace{2cm}} this book?
    \begin{enumerate}
        \item \alt<2->{\Circled{read}}{read}
        \item reading
        \item reads
        \item to read
    \end{enumerate}
...
% Q2: visited (語彙と形)
    \item あなたはこれまでに何回、京都を訪れましたか\\
    How many times have you \underline{\hspace{2cm}} Kyoto?
    \begin{enumerate}
        \item visit
        \item \alt<3->{\Circled{visited}}{visited}
        \item visiting
        \item visits
    \end{enumerate}
\end{enumerate}
\hfill{\tiny 0144}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_061.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{hearとlisten to}
\large
 \begin{enumerate}
  \item Did you \textbf{hear} the strange sound?
  \item I \textbf{listened to} the radio.
 \end{enumerate}
...
使いわけは\pause
\begin{description}[　　　　　　]
 \item[hear] 自然に聞こえてくる\pause
 \item[listen] 意識的に耳をかたむける\\\pause
前置詞のtoをセットにする
\end{description}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}
% Q3: listen to (前置詞と過去分詞)
    \item あなたはその歌を何回聞いたことがありますか。\\
    How many times have you \underline{\hspace{2cm}} to the song?
    \begin{enumerate}
        \item heard
        \item listen
        \item \alt<2->{\Circled{listened}}{listened}
        \item hearing
    \end{enumerate}
    
    % Q4: 回数の答え方
    \item 質問に対する答えとして最も適切なものを選びましょう\\
    Q: How many times have you seen the movie?\\
    A: \underline{\hspace{2cm}}.
    \begin{enumerate}
        \item Yes, I have.
        \item \alt<3->{\Circled{Three times.}}{Three times.}
        \item Last Sunday.
        \item For two hours.
    \end{enumerate}
\end{enumerate}
\hfill{\tiny 0130}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_062.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]{Exercises}
 日本文の意味になるように、( )内の語句を並べ替えて正しい英文を完成させましょう。\\
なお、先頭の語は大文字で始めてください

\begin{enumerate}
    % Q5: 基本語順
    \item あなたはこの映画を何回見たことがありますか。\\
    ( seen / times / you / many / how / have ) this movie?
    \\
    \visible<2->{How many times have you seen the movie?}

    % Q6: eat natto
    \item あなたは今までに何回、納豆を食べたことがありますか。\\
    ( eaten / you / times / natto / have / how / many ) ?
    \\
    \visible<3->{How many times have you eaten natto?}

    % Q7: climb Mt. Fuji
    \item あなたは富士山に何回登ったことがありますか。\\
    ( climbed / many / you / how / Mt. Fuji / times / have ) ?
    \\
    \visible<4->{How many times have you climbed Mt. Fuji?}
\end{enumerate}
\hfill{\tiny 0144}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_063.mp3}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Listen, Then Read\,\,\,\,\,{\tiny 0051}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_reading.mp3}}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
 
\includegraphics[width=1.01\textwidth]{./images/nanobanana-output/013_have_pp_keiken_reading.png}

\vspace{-15pt}

\hfill{\tiny 0051}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_reading.mp3}}
...
before upper={\setlength{\parindent}{1.25em}},
 title=My Travel Experiences\mbox{}\hfill{\tiny 0051}\,{\scriptsize \myaudio{./audio/013_have_pp_keiken_reading.mp3}}
]
\noindent{}I love traveling\footnotemark[1].
I have visited Hawaii twice.
The ocean is beautiful.
I have eaten big pancakes there many times.
They are delicious\footnotemark[2].

However\footnotemark[3], I have never visited Europe.
My dream is to go to Italy.
I want to see the Colosseum\footnotemark[4].
I have studied Italian history once, but I want to learn more.

How about you?\footnotemark[5] Have you ever traveled to a foreign country?
...
\scriptsize
$^{1}$travel：旅行する\hfill{}$^{2}$delicious：おいしい\hfill$^{3}$however：しかし

$^{4}$the Colosseum：コロッセオ（古代ローマ帝国時代に建設された巨大な円形闘技場）

$^{5}$How about you?：あなたはどうですか？
...
\includegraphics[width=.975\textwidth]{./images/nanobanana-output/013_have_pp_keiken_reading_colosseum_now.png}
...
\includegraphics[width=.975\textwidth]{./images/nanobanana-output/013_have_pp_keiken_reading_colosseum_then.png}
...
\begin{tcolorbox}[title=わたしの旅の経験]
　わたしは旅行が大好きです。
わたしはハワイを2回訪れたことがあります。
海がうつくしいです。
そこで大きなパンケーキを何度も食べたことがあります。
それらはとてもおいしいです。

　しかし、わたしはヨーロッパは一度も訪れたことがありません。
わたしの夢はイタリアに行くことです。
コロッセオを見たいとおもっています。
わたしはイタリアの歴史を一度勉強したことがありますが、もっと学びたいです。
...
\includegraphics[width=1.01\textwidth]{./infographic/013_have_pp_keiken_infographic.png}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
 
\includegraphics[width=1.01\textwidth]{./infographic/013_have_pp_keiken_infographic.png}
\hfill{\tiny audio\_overview 1752}\,{\scriptsize \myaudio{./audio/overview/013_have_pp_keiken_audio_overview.m4a}}%
\hfill{\tiny eduop\_boys 0324}\,{\scriptsize \myaudio{./audio/manzai/013_have_pp_keiken_manzai.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
...
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/013_have_pp_keiken_test.png}
 \end{column}
...
\url{https://forms.gle/JCAbtwtmjVLhXs1w6}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
