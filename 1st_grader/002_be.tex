...
\begin{frame}<1-27>[plain]\frametitle{「AはBだ」という表現}
 % \setbeamercovered{transparent}
  \begin{enumerate}
   \item<1-> I \textbf<14-20>{\color<14-20>{red}{am}} a student. \onslide*<2>{わたしは生徒です。}\onslide*<14-20>{(I $=$ a student)}\onslide*<21-26>{\footnotesize 　a:１つの、1人の　student: 生徒、学生}
...
\onslide<20->{\begin{exampleblock}{ポイント}
  am, are, isはイコール($=$)の意味。まとめてbe動詞といいます
     \end{exampleblock}
}
...
\begin{frame}[plain]\frametitle{am, are, is --- みんな、なかまです $\longrightarrow be動詞といいます$}
 \centering
\begin{tikzpicture}
% 補助グリッドを描画
...
% ノード間の線の描画
\draw[-latex, line width=1.5pt] (A) -- node[above] {hoge} (B);
\draw[-latex, line width=1.5pt] (A) -- node[sloped, above] {oyoyo} node[sloped, below] {tohoho} (C);
\draw[-latex, line width=1.5pt] (A) -- node[above] {piyopiyo} (D);
\end{tikzpicture}
\end{frame}
...
{\LARGE
\begin{numcases}{\text{be動詞の使い分け}}
 \text{Iではじまる}&\text{am}\\
 \text{Youではじまる}&\text{are}\\
 \text{1つ、1人}&\text{is}\\
 \text{2つ、2人以上}&\text{are}
\end{numcases}
}
