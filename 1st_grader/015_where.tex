...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
\section{Listen, Then Read\,\,\,\,\,\,{\tiny 0056}\,{\scriptsize \myaudio{./audio/015_where_reading.mp3}}}
%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
...
scramble \textipa{/skr\'\ae mbl/} 大あわて\hfill{\tiny 0056}\,{\scriptsize \myaudio{./audio/015_where_reading.mp3}}
...
\noindent{}It is Sunday morning.
Ken is very busy.
He cannot find his things.

He asks his mother, ``Where is my red cap?''
She says, ``It is on\tikzmark{clue1} the chair.''
Next, Ken asks his father, ``Where are my shoes?''
His father says, ``They are by\tikzmark{clue2} the door.''

Ken has a small dog named Pochi.
``Where does Pochi sleep?''
Ken asks. ``He sleeps under\tikzmark{clue3} the table,'' says his sister.
``Where do you go now?'' asks his mother.
``I go to the \tikzmark{clue4}park with Pochi!'' says Ken.
...
\begin{enumerate}\setlength{\itemsep}{-2pt}
 \item<2-> The cap is under the table.\tikzmark{q1}\hfill\visible<3->{F}
 \item<2-> The shoes are by the door.\tikzmark{q2}\hfill\visible<4->{T}
 \item<2-> Pochi sleeps on the chair.\tikzmark{q3}\hfill\visible<5->{F}
 \item<2-> Ken goes to the park with his dog.\tikzmark{q4}\hfill\visible<6->{T}
\end{enumerate}
...
\begin{tikzpicture}[remember picture,overlay]
 ...
 \visible<5->{\draw[<-,line width=2pt, opacity=.4] ([yshift=-2pt]pic cs:clue3) to[bend left] ([xshift=2pt, yshift=2pt] pic cs:q3);}
 \visible<6->{\draw[<-,opacity=.4,line width=2pt] ([yshift=-2pt]pic cs:clue4) to[bend left] ([xshift=2pt, yshift=2pt] pic cs:q4);}
\end{tikzpicture}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%このスライドは現在形に加えて過去形や未来形の例文もある。
%%%%%%%既習事項に考慮して適宜コメントアウトする。
%%%%%%%なお音声ファイルには過去形・未来形の例文も収録してあるので、
%%%%%%%コメントアウトしたときは音声ファイルを別途作成すること
...
\hfill {\tiny audio\_overview 0252}\,{\scriptsize \myaudio{./audio/manzai/015_where_manzai.mp3}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/015_where_test.png}
 \end{column}
...
\url{https://forms.gle/bppYDA7sCYRTdBvg8}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
