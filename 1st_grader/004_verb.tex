...
\hfill{\tiny 0253}\,{\scriptsize \myaudio{./audio/004_verb_01.mp3}\,01}
...
\onslide*<1->{つぎの文の主語は何人称ですか？}\hfill{\tiny 0253}\,{\scriptsize \myaudio{audio/004_verb_02.mp3}\,02}

 % \setbeamercovered{transparent}
  \begin{enumerate}
   \item<1-> \myEmph[2,8]{Maroon}{I} am a student. \onslide*<2-10>{{\small わたしは生徒です。}}\hfill\onslide*<11->{\scriptsize 　a:１つの　student: 生徒、学生}
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{主語によって述語動詞のかたちが定まる}
\begin{frame}[plain]\frametitle{主語によって述語動詞のかたちが定まる}
 % \setbeamercovered{transparent}
  \begin{enumerate}
...
\begin{itemize}\setbeamertemplate{items}[square]
 \item<10-> 主語が1人称のとき、述語動詞はそのまま\hfill{I \textbf{sing}. / We \textbf{sing}.}
 \item<11-> 主語が2人称のとき、述語動詞はそのまま\hfill{You \textbf{sing}.}
 \item<12-> \myEmph[14-]{Maroon}{主語が3人称で単数のとき、述語動詞 $=$ \Circled[fill color=white]{\,原形 $+$\,s\,}}\hfill{She \textbf{sings}. / He \textbf{sings}.}
 \item<13->[] \hspace{67.25pt}複数のとき、述語動詞はそのまま\hfill{They \textbf{sing}.}
\end{itemize}
      \end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
主語が\myEmph[2]{Maroon}{3人称}\myEmph[2]{Maroon}{単数}のとき、原形の最後に\myEmph[2]{Maroon}{s}をつけます
...
\mbox{}\hfill{\tiny 0135}\,{\scriptsize \myaudio{audio/004_verb_021.mp3}\,021}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Embed the sound file
\mbox{}\hfill{\tiny 0228}\,{\scriptsize \myaudio{audio/004_verb_03.mp3}\,03}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain,t,label=root_or_present]{be動詞と一般動詞 / 原形と現在形}
 
\begin{tabular}{llll}\toprule
 動詞&原形 &現在形 &主語\\\midrule
 \visible<2->{be動詞}&\visible<3->{be}&
    \visible<4->{$\left\{\begin{tabular}{ll}
     am&\\
     are&\\
     is&
    \end{tabular}\right.$} &
    \visible<5->{$\left.\begin{tabular}{l}
     I\\
     you / （\,I / you以外の）複数\\
     \hspace{29.5pt}（\,I / you以外の）単数\\
    \end{tabular}\right.$} \\\midrule
\visible<6->{一般動詞}&\visible<7->{like}&
    \visible<8->{$\left\{\begin{tabular}{ll}
     like\,\,\,($=$\,原形と同じ)& \\
     likes\,($=$\,原形 $+$ s\,)& \\
    \end{tabular}\right.$} &
     \visible<9->{$\left.\begin{tabular}{l}
     三人称単数以外\\
     三人称単数\\
...
\visible<10->{{\small Q. つぎの英文の下線部の動詞は原形か現在形か、答えてください}
\vspace{-5pt}
\begin{enumerate}
 \item He \underline{is} very kind.
 \item \underline{Are} you hungry?  
 \item She \underline{likes} music.
 \item I \underline{like} sushi. 
\end{enumerate}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{注意するべき3単現}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}\setbeamertemplate{items}[square]
 \item<1-> \myEmph[13]{Maroon}{3単現の原則：\Circled[fill color=white]{\,原形 $+$ s\,}}\\
\mbox{}\hspace{70pt}\onslide<14->{{\scriptsize  例えば}{\small drinks, eats, likes, lives, plays, speaks, walks\ldots{}\,\,{\scriptsize $\leftarrow$覚える必要なし}}}
 \item<2-> \onslide<15->{\myEmph[15]{Maroon}{一部の動詞：\Circled[fill color=white]{\,原形 $+$ es\,}}}\\
\mbox{}\hspace{70pt}\onslide<16->{{\small goes, teaches, washes, watches}\,\,{\small $\leftarrow$ まずはこれだけ覚えよう！}}
 \item<3-> \onslide<17->{\myEmph[17]{Maroon}{haveの3単現：\Circled[fill color=white]{\,has\,}}\hspace{20pt}}\onslide<18->{\small $\longleftarrow$\, これも覚えよう！\myCrossedOut{Maroon}{haves}}
\end{itemize}
      \end{block}

\vspace{-16pt}

\mbox{}\hfill{\tiny 0300}\,{\scriptsize \myaudio{audio/004_verb_04.mp3}\,04}
...
&原形&3単現&発音\\
1&\visible<1->{play}&\visible<2->{plays}&\visible<9->{\textipa{/z/}}\\
2&\visible<1->{drink}&\visible<3->{drinks}&\visible<10->{\textipa{/s/}}\\
3&\visible<1->{go}&\visible<4->{goes}&\visible<12->{\textipa{/z/}}\\
4&\visible<1->{teach}&\visible<5->{teaches}&\visible<13->{\textipa{/\textsci{}z/}}\\
5&\visible<1->{wash}&\visible<6->{washes}&\visible<14->{\textipa{/\textsci{}z/}}\\
6&\visible<1->{watch}&\visible<7->{watches}&\visible<15->{\textipa{/\textsci{}z/}}\\
7&\visible<1->{have}&\visible<8->{has}&\visible<17->{\textipa{/h\'\ae z/}}\\
\end{tblr}%
}
\end{center}
...
\mbox{}\hfill{\tiny 0422}\,{\scriptsize \myaudio{audio/004_verb_041.mp3}\,041}
...
\mbox{}\hfill{\tiny 0306}\,{\scriptsize \myaudio{audio/004_verb_05.mp3}\,05}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}<15-18>[plain,label=hyo]{注意するべき3単現}
...
1&\visible<1->{play}&\visible<2->{plays}&\visible<9->{\textipa{/z/}}\\
2&\visible<1->{drink}&\visible<3->{drinks}&\visible<10->{\textipa{/s/}}\\
3&\visible<1->{go}&\visible<4->{goes}&\visible<11->{\textipa{/z/}}\\
4&\visible<1->{teach}&\visible<5->{teaches}&\visible<12->{\textipa{/\textsci{}z/}}\\
5&\visible<1->{wash}&\visible<6->{washes}&\visible<13->{\textipa{/\textsci{}z/}}\\
6&\visible<1->{watch}&\visible<7->{watches}&\visible<14->{\textipa{/\textsci{}z/}}\\
7&\visible<1->{have}&\visible<8->{has}&\visible<15->{\textipa{/h\'\ae z/}}\\
8&\visible<16->{study}&\visible<17->{studies}&\visible<18->{\textipa{/z/}}\\
\end{tblr}%
}
\end{center}
...
\vspace{-40pt}
\visible<16-18>{\raisebox{\height}{\dbend}\,studyを追加しよう$\longrightarrow$}

\mbox{}\hfill{\tiny 0541}\,{\scriptsize \myaudio{audio/004_verb_06.mp3}\,06}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{まとめ}
\againframe<11>[plain]{point1}
\againframe[plain]{figure}
\againframe<2,4,14>[plain]{ninsyo}
\againframe<5>[plain]{3tangen}
\againframe<19>[plain]{hyo}
\againframe<9>[plain]{root_or_present}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[plain]
 
\includegraphics[width=1.01\textwidth]{./infographic/004_verb_infographic.png}
\hfill{\tiny audio\_overview 1021}\,{\scriptsize \myaudio{./audio/overview/004_verb_audio_overview.m4a}}%
\hfill{\tiny eduop\_boys 0351}\,{\scriptsize \myaudio{./audio/manzai/004_verb_manzai.mp3
}}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%
...
\begin{columns}
 \begin{column}{0.4\textwidth}
  \includegraphics[width=1.1\textwidth]{./test_forms/004_verb_test.png}
 \end{column}
...
\url{https://forms.gle/RdVDCQPZBaY1pWUB7}
\end{frame}
%%%%%%%%%%%%%%%%%%%
\end{document}
