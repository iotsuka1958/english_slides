...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}<1-8>[plain,label=im]{Answer}
 \Large
...
\begin{enumerate}
 ...
 \item {\small あなたは親切だ。}(\alt<1-3>{~~\phantom{You're}~~}{~~\textcolor{Maroon}{You're}~~})~~kind.
 \item {\small あなたは背が高い。}(\alt<1-4>{~~\phantom{You're}~~}{~~\textcolor{Maroon}{You're}~~})~~tall.
\end{enumerate}
...
\begin{itemize}\setbeamertemplate{items}[square]
 ...
 \item<8-> \textbf{You are} $=$ \textbf{You're}\tikzmark{target2} \\
\mbox{}\hfill{}{\scriptsize \tikzmark{pointer2}短縮形といいます。 \Circled[fill color=white]{~'~}\,をapostrophy\,(アポストロフィ)といいます\phantom{あ}}
\end{itemize}
     \end{block}
...
\begin{tikzpicture}[remember picture,overlay]
 \visible<9->{\draw[<-,opacity=.4,line width=2pt] ([yshift=-2pt]pic cs:target2) to[out=-15, in=180] ([xshift=-2pt, yshift=2pt] pic cs:pointer2);}
\end{tikzpicture}
...
\hfill{\tiny 0200}\,{\scriptsize \myaudio{audio/002_be_041.mp3}}{\scriptsize それぞれ2行目の短縮形の練習です}
