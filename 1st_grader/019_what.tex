...
\draw[->, line width=3pt, opacity=.5, orange] (focus.south) to[out=-90, in=90] node[sloped,above,text=black,font=\tiny,pos=.667]{先頭へ} node[sloped,below,text=black,font=\tiny,pos=.5]{後は疑問文の語順} (wh.north);
\end{tikzpicture}
}
...
\visible<5->{%
\begin{tikzpicture}[remember picture, overlay]
 \draw[line width=3pt, opacity=.5, orange, ->] (focus2.south) to[out=-90, in=90] node[sloped,above,text=black,font=\tiny,pos=.667]{先頭へ} node[sloped,below,text=black,font=\tiny,pos=.5]{後は疑問文の語順} (WH2.north);
\end{tikzpicture}
}
...
\visible<3->{What books do you read?}
 \item ( hobbies / you / have / do / what ) ?\hfill{}{\scriptsize hobby \textipa{/h\'Abi/}}\\
...
\visible<5->{What day is it today?}\hfill{}\visible<6->{{\small \textdbend\,What day of the week is it today?}}
\end{enumerate}
