...
\myAnch{s-1}{orange}{You} \myAnch{be-1}{olive}{are} happy.\raisebox{-10pt}{\scalebox{2}{\myDownwardPitch}}
\vspace{35pt}\pause
...
\pause
\node at (-3,-1) {{\scriptsize \textbullet\,\,be動詞を先頭に(Sはその直後)}};
\pause
\node at (0.5,0.25) {\scalebox{2}{\myRisingPitch}};
\pause
\node at (2,-1) {{\scriptsize \textbullet\,\,?と最後のイントネーションに注意}};
% Embed the sound file
\node at (7,-1.5) {\myaudio{./audio/023_is_ing_question_000.mp3}};
\end{tikzpicture}
...
つぎの文を疑問文にしましょう
 ...
 \begin{enumerate}
  \item<1-> I am late for the meeting.
        \onslide<5->{$\longrightarrow$ Am I late for the meeting?}\hfill\onslide<1->{{\scriptsize be late for ～に遅れる}}
  \item<1-> You are hungry.
        \onslide<6->{$\longrightarrow$ Are you hungry?}\hfill\onslide<1->{{\scriptsize hungry: 空腹だ}}
 \item<1-> They are happy.
  ...
  \item<1-> Our teacher is busy.
        \onslide<9->{$\longrightarrow$ Is our teacher busy?}\hfill\onslide<1->{{\scriptsize busy \textipa{/b\'Izi/} いそがしい}}
 \end{enumerate}

\begin{exampleblock}<2->{Topics for Today}\small
be動詞の疑問文のつくり方
\begin{itemize}
 \item<3->  be動詞が先頭（主語はその後ろ）\\
...
\end{itemize}
      \end{exampleblock}
\hfill{}\visible<8->{{\scriptsize be動詞の疑問文のときと同じです}}
...
\begin{enumerate}
 \item ( Do / Does / is / \alt<2->{\Circled[outer color=orange]{Is}}{Is} ) she reading a book?\hspace{10pt}\raisebox{0pt}{\bcbook}
 \item ( Do / Does / \alt<3->{\Circled[outer color=orange]{Are}}{Are} / is ) you waiting for the bus?\hspace{10pt}\scalebox{2.5}{\twemoji{bus stop}\twemoji{bus}}\hfill{\scriptsize wait for ～を待つ}
 \item Are you ( playing / \alt<4->{\Circled[outer color=orange]{listening}}{listening} / listen ) to music now?\hfill{\scriptsize listen to ～を聞く}\\
...
\mbox{}\hfill{\scriptsize \myaudio{./audio/023_is_ing_question_04.mp3}}
\end{frame}
...
\hfill{}{\normalsize She {\bfseries is} \Circled[fill color=white]{\,not\,} {\bfseries sleeping} now.}
 \item 疑問文のつくり方: be動詞を先頭に
\hfill{}{\normalsize {\bfseries Is} she {\bfseries sleeping} now?}\\
\hfill{}Yes, she is.\\
\hfill{}No, She is not.\\
\hfill{}($=$ No, she's not.)\\
\hfill{}($=$ No, she isn't.)
