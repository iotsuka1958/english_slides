...
\begin{exampleblock}<11->{Topics for Today---複数形のsの発音---}
...
\end{description}
\end{exampleblock}
\hfill\onslide<11->{{\scriptsize 複数形のsの発音と同じかんがえかたです}}
\end{frame}
...
\end{tabular}
\dbend}\\
\hfill\onslide<15->{\scriptsize 母音字: aeiou\hspace{10pt}子音字: それ以外}
\end{exampleblock}
...
\begin{enumerate}
 \item<1-> key \onslide<2->{$\longrightarrow$ keys}
 \item<1-> monkey \onslide<3->{$\longrightarrow$ monkeys}
 \item<1-> body \onslide<4->{$\longrightarrow$ bodies}
 \item<1-> story \onslide<5->{$\longrightarrow$ stories}
 \item<1-> family \onslide<6->{$\longrightarrow$  families}
\end{enumerate}
\end{frame}
%%%%%%%%%%%%%%%%%%
...
\end{description}
\end{exampleblock}
\hfill\onslide<7->{{\scriptsize yで終わる名詞の複数形のつくり方と同じです}}
