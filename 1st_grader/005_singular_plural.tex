...
\end{description}
\end{exampleblock}
\myaudio{./audio/005_singular_plural_a.mp3}\hfill\onslide<11->{{\scriptsize 複数形のsの発音と同じかんがえかたです}}
\end{frame}
...
\onslide<14->{$\longrightarrow$
\begin{tabular}[t]{ll}
 母音字なら&原則どおり(sをつけるだけ) \\
 子音字なら&yをiに変えてes 
\end{tabular}
\dbend}\\
\myaudio{./audio/005_singular_plural_b.mp3}\hfill\onslide<15->{\scriptsize 母音字: aeiou\hspace{10pt}子音字: それ以外}
\end{exampleblock}
...
\begin{enumerate}
 ...
 \item<1-> body \onslide<4->{$\longrightarrow$ bodies}\hfill{}{\scriptsize body \textipa{/b\'di/} 身体}
...
\scalebox{.5}{\fcBus{0.4}{black}{2}}\hspace{15pt}\pause {\LARGE a bus} {\Large \textipa{/b\'2s/}}
\pause
...
\pause
{\LARGE a potato}\Large \textipa{/p@t\'eItoU/}{} 
\pause
