...
\hfill{\tiny 0424}\,{\scriptsize \myaudio{audio/022_is_ing_negative_01.mp3}}
...
\begin{itemize}\setbeamertemplate{items}[square]
 \item<2->  be動詞の否定は\,\,\Circled[fill color = white]{\,\,$\text{be動詞} + \text{\textcolor{Maroon}{\bfseries not}}$\,\,}
...
\begin{enumerate}
 \item<3-> They {\itshape are} {\bfseries not} students. \onslide<5->{($=$ They {\bfseries aren't} students.)}
 \item<3-> She {\itshape is} {\bfseries not} busy. \onslide<5->{($=$ She {\bfseries isn't} busy.)}
 \item<3-> I {\itshape am} {\bfseries not} hungry. \onslide<5->{($\neq$ * I {\bfseries amn't} hungry.)}
\end{enumerate}
\end{minipage}
\end{block}
\end{frame}
