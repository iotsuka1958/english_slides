...
気温はきのうまでほど高くはありませんが、
雨のせいか湿度が高くなっていて、快適にはほど遠い体感です。
熱中症警戒アラートはでていないようですが、
...
\begin{verbatim}
ffmpeg -i hoge.mp4 -vf ”subtitles=hoge.srt:charenc=UTF-8” -c:v libx264 -crf 18 -preset slow -c:a copy hoge\_subtitle.mp4
\end{verbatim}
...
結果が出力されるので、それをコピー安堵ペーストして、
たとえばhoge\_ja.srtというファイル名にして次のコマンド打てば日本語字幕入りmp4が完成

\begin{verbatim}
ffmpeg -i hoge.mp4 -vf ”subtitles=hoge_ja.srt:charenc=UTF-8” -c:v libx264 -crf 18 -preset slow -c:a copy hoge_ja_subtitle.mp4
\end{verbatim}
